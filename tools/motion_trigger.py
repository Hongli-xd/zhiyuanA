...
def pick_motion_for_response(text: str) -> Optional[str]:
    ...
    优先级顺序（最具体优先）：
      1. 手势类（握手/点赞/加油/挥手/比耶/OK/NO）—— 最具体，不易误触发
      2. 数字
      3. 引导方向
      4. 欢迎/礼仪
      5. 讲解结构
    ...
    Returns:
        动作名（如"结尾致谢_7s"），找不到返回 None
    """
    # ── 1. 手势类（最具体，不易误触发）──────────────
    if _match(r"握个手", text): return "右手握手"
    if _match(r"点赞", text): return "右手点赞"
    if _match(r"加油", text): return "双手加油"
    if _match(r"比心", text): return "双手向下比心"
    if _match(r"挥手", text): return "双手挥手"
    if _match(r"比耶", text): return "双手比耶"
    if _match(r"OK手势", text) or _match(r"比OK", text): return "右手OK"
    if _match(r"NO手势", text) or _match(r"比NO", text): return "右手NO"
    ...
    if _match(r"欢迎来到", text) or _match(r"欢迎各位", text): return "向右欢迎"
    if _match(r"欢迎大家", text): return "向左欢迎"
    if _match(r"自我介绍一下", text) or (_match(r"大家好", text) and _match(r"我叫", text)): return "开篇打招呼_12s"
    if _match(r"敬礼", text): return "敬礼"
...
def _has_counting(text: str) -> bool:
    """text 中是否含明确的计数词（排除"一号产品"这类）。"""
    # 匹配 "一个/两个/三个" 或 "第一/第二/第三" 或 "一、二、三" 或 "个要点"
